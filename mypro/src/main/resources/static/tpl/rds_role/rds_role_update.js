...
function initZTree(){
    $.ajax({
        ...
        success: function(data){
            ...
            $.ajax({
                url:"/rds_role/getRdsRoleMenuByZtree?roleId="+updateForm.roleId.value,
                type:"post",
                dataType: "json",
                success: function(data){
                    var nodes=data.data;
                    ...
                    for(var i=0;i<nodes.length;i++){
                        $("#menuIds").append("<input name='menuIds' type='hidden' value='"+nodes[i].menuId+"'/>");
                        zTreeObj.checkNode(zTreeObj.getNodeByParam("id",nodes[i].menuId), true );
                    }
